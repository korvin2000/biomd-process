...
**Status: v0.6 — platform complete, domain implemented, sources beyond the article, no document left behind, the corpus is not only soloists, and the work is scheduled rather than queued.** Orchestration, routing, reliability, cost control, resume and observability were done in v0.1 and are largely unchanged; v0.4 closed the three ways a single document could still be lost — a routing decision that only looked at the context window, a pool with nothing to fall back to, and a corpus-scope dependency that let one failure retire the whole catalogue. v0.2 filled in the domain against the normative specification: the format lives in `src/domain` and nowhere else, and `biomd validate` checks the published output against its invariant list. v0.3 added the two sources of fact an article does not contain — an image index (`portrait`) and the web (`websearch`) — and lowered the date floor so a year-only date survives instead of being dropped. v0.4 made a failure local: it can cost the entry the field, the edition or the row it belongs to, never the catalogue. v0.5 addressed what a corpus of a thousand real articles turned out to contain that four sample biographies did not — **collectives** (a duo's portrait has two faces in it, a trio has no date of birth, and `gender: mixed` is what the format calls one), **a second name source** (`data/names.json`, the site's own extracted name list), and **quoted foreign text**, which a Russian article is full of and which must survive translation unchanged. v0.6 is about *when* and *where* the work goes rather than what is in it: three settings that all mean "this output already exists" are now read before a model is called instead of after it is billed; a routing pool chooses its own strategy, because extraction is scarce in money and translation in wall-clock; and translation is scheduled across every endpoint at once (`least-busy` + per-pool concurrency lanes + a per-language preferred model) instead of queueing on whichever one is cheapest — measured at 1m 43s → 34s on three real articles. Read [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) before making structural changes.
...
`RoutingStrategy.select(ctx): ModelTarget[]` ranks a pool and never calls anything (`src/routing/strategies/builtin.ts`: `cost-optimized`, `context-optimized`, `sequential`, `round-robin`, `least-failures`, `least-busy`). Pools (`llm.routing.pools` in config) let `extract` and `translate` route to different model sets — cheap models for extraction, a strong one for translation. Custom strategies register via `app.strategies.register(defineStrategy(id, description, select))`.

**A pool is written as a list or as an object, and the object is where the three per-pool decisions live.** `translate: [a, b, c]` is the shorthand every config used before and still means exactly what it did. The expanded form adds `strategy`, `maxConcurrent`, `prefer` and `options` — each of them a thing a *global* setting cannot express, because the stages of this tool are scarce in different resources: extraction in money, translation in wall-clock, web search in "a model that can actually search". One global `strategy:` forced all three to pretend otherwise; it is now the **default** a pool inherits rather than the only answer. A per-pool strategy id that does not resolve fails when the app is built, not on the one call that happens to use that pool.

**The scarce resource in a translation run is time, and the shape of the waste is a queue.** `cost-optimized` ranks the free local model first for every request, which is right about price and, at `run.concurrency: 3` against an endpoint that serves one request at a time, means three concurrent tasks become one while two other endpoints sit idle. `least-busy` ranks the **emptiest endpoint** first and breaks the tie on cost — so an untouched pool behaves exactly like `cost-optimized` and only starts to differ once something is actually busy. Measured on the real deployment (2026-08-23, three articles into English, `run.concurrency: 3`): **1m 43s → 34s**, same output, $0.00012 of it paid. That is not faster models; it is three of them working instead of one.

It ranks on **how full** an endpoint is, never on how many slots it has left. An endpoint that allows three parallel requests has three free slots while idle and one that allows a single request has one, so the count would hand the generous endpoint every request from the first one onwards — precisely the imbalance the strategy exists to prevent. As a fraction of what each can hold, idle is idle.

**Concurrency is counted twice, on purpose** (`src/llm/Lanes.ts`). An endpoint's `maxConcurrent` is a fact about the provider — three is what OpenRouter tolerates, one is what `omniroute` requires for *correctness*. A **lane** (`llm.routing.pools.<pool>.maxConcurrent.<endpoint>`) is this pool's share of that budget, and the config schema refuses lanes that add up to more than the endpoint allows: a lane divides a cap, it never raises one. A pool's free capacity is whichever of the two runs out first, so an endpoint another pool is saturating correctly reads as full.

Two mechanisms enforce it and they are not interchangeable. **Claims** are the routing input: the gateway claims a lane in the *same synchronous tick* as the ranking that chose it, which is the whole difference between spreading load and merely intending to — without it, three tasks starting together all read "the local model is free", all pick it, and two block on a semaphore they could have avoided. **Semaphores** are the enforcement, and they still apply when the chain has nowhere else to go. Ranking is advice; a cap is a cap. The lane semaphore is acquired *before* the endpoint's, never after, so a pool whose lane is full never occupies an endpoint slot while waiting.

**`prefer` is a language naming its own model.** `llm.routing.pools.<pool>.prefer.<variant>` floats models to the front for one task variant — which for `translate` and `localize` is the target language. A Chinese open-weights model renders Chinese and Japanese better than a Western one and the reverse holds for European languages; that is a fact about the language, not about the run. It is applied *after* the strategy has ranked, so quality outranks both price and queue depth, and it is a **reordering, never a filter**: the rest of the pool stays behind the preferred model as its fallback chain, so a preference can slow a language down and can never make it unroutable. A `prefer` entry naming a model the pool does not contain is a config error rather than a line that silently does nothing.
...
**Fallback one level up: the answer was wrong, not the call** (`reliability.taskFallback`, `src/core/AttemptScope.ts`, `Orchestrator.executeTask`). Everything above answers *this call failed*. None of it can answer *every call returned 200 and the document they add up to is broken* — only the task that assembled the document knows that, and it arrives as a pipeline error long after the last successful response. It used to end the task on the spot with two untried models sitting in the pool.

A task is therefore run again, and what varies between attempts is deliberately small. The targets that already failed it are **demoted** (`RoutingRequest.avoid`), so a fresh one leads the chain; the last attempt — which by then has nobody new to ask — changes *how* it asks instead of whom, via `lastAttempt.strategy` and `lastAttempt.temperature`. Demotion rather than exclusion, for the same reason `onOverflow: demote` exists: a pool of three that has used all three still has to route somewhere. It is applied *after* `prefer`, and that order is the point — a language preference is a claim about quality, and a model that just produced a broken answer for this very task is evidence.

`ExecutionContext.llm` is an `LlmPort` rather than the gateway itself so the orchestrator can wrap it: `AttemptScope` carries the tuning into every call the task makes and records who answered. That is what gave `extract`, `translate`, `websearch` and `localize` this at once, without a line changing in any of them. A pipeline holding a cache does need to know, though, and gets `ExecutionContext.attempt`: **a retry must not be served the previous attempt's answers**, because they are what failed — without that, every fragment of a re-translated document is a translation-memory hit, no model is called, and the second attempt rebuilds the identical broken document for free.

Two failures are deliberately *not* retried. One that never reached a model is deterministic — an unreadable source, a bad path template — and three goes at it is three times the wall-clock for the same error. And a call that exhausted its chain has already had this treatment one level down, so re-running meets the same chain in a worse state and the run's report ends up naming an open circuit instead of the failure that opened it. The exception is the reason the mechanism exists: when those targets failed by **answering badly** (`response_format`) rather than by failing, the models are alive and the answers were wrong, and asking again more literally is a genuinely different question. That distinction — a technical error versus an unusable result — is the only place the two need telling apart.
...
3. **Send prose, not documents.** `extractTextSpans` (`src/documents/markdown/textSpans.ts`) and the dossier-side equivalent in `src/pipelines/localization/StringTable.ts` pull out only translatable text; headings, list markers, `:::` containers and their attributes, fenced code, URLs, and — for dossiers — `dates`/`ranking`/`url`/media targets/unknown fields never leave the machine. What crosses the wire is a flat `{contentHash: text}` table (`tasks.translate.mode: segments`, the default); the model must return exactly the same keys, so a dropped or invented key is caught rather than silently written out. Repeated strings are translated once via `TranslationMemory`; `tasks.*.useTranslationMemory: persistent` keeps that cache across runs in `run.memoryDir`, namespaced by prompt version so a prompt edit starts a fresh one. Link targets inside a sentence are masked as `⟦1⟧` and checked for survival before acceptance. Whole-document mode (`tasks.translate.mode: document`) still exists for when maximum surrounding context matters more than cost.
   ...
   **A fragment can change the shape of the line it goes back on**
   (`structuralDrift`, `escapeBlockMarker` in
   `src/documents/markdown/textSpans.ts`). Segment mode's premise is that the
   model never sees the markup and so cannot damage it, and the premise is very
   nearly true: what survives it is the replacement text. A paragraph answered
   as `1. Estudio guitarra...` is an ordered list item the moment it is spliced
   back, and one answered on two lines splits a block in half. Neither is
   visible to the `{hash: text}` contract, so both used to be caught only by the
   whole-document guard — after every call in the batch was paid for, as a
   failed article with no edition on disk. Measured on this corpus:
   `bitetti → en`, `blackmore → de` and `belousov → es` all died that way while
   their eleven sibling editions published normally.

   The two halves of the answer are not the same, and telling them apart is the
   whole design. **Structure the model invented** — a heading, a quote, a
   `key:` prefix on a line that was prose — is a mistake, so `structuralDrift`
   reports it through the existing per-fragment `verify` hook, which runs
   *inside* the gateway's `validate` and therefore inherits the cheap per-key
   repair ladder and then retry-then-fall-back-to-another-model. **A list
   marker is not a mistake**: `27 марта 2002 г.` is one paragraph and its German
   is `27. März 2002`, which is correct, is what all three models answer, and is
   also `oli` at the start of a line. Nine calls across three models were spent
   re-asking for a right answer already given. CommonMark already settles it, so
   `escapeBlockMarker` takes the backslash rather than asking — at the splice,
   where the span's kind is still known and where "safe to put back on a line of
   its own" is the actual question. The same document now translates in four
   calls with no retries and a skeleton identical to the source.
...
8. **The cheapest call is the one not made, and three settings say so.** `output.onExisting: skip`, `run.skipExistingOutputs: true` and `tasks.extract.onExistingDossier: reuse` are three different reasons not to spend, and they are all read at **plan** time now. `onExisting: skip` was the one that was not: it is a promise the *writer* makes ("this file will not be replaced"), so a task whose output exists was run, billed, and then had its answer discarded with a warning — ruinous after a prompt edit, which invalidates every fingerprint and therefore re-plans the whole corpus. The planner reads it as what it is: a guarantee that running the task changes nothing. `overwrite` and `fail` are untouched — one intends to replace the file, the other to stop the run loudly. A task that declares `mergesOutput` is exempt from both, because its file exists from the end of the first run onwards and says nothing about whether this run's work is in it.

`tasks.extract.onExistingDossier: reuse` (the default) re-emits an **authored** `<slug>.bio.json` — the one beside the article, normalized and migrated to version 2 — for **zero tokens**; `complete` asks only for the keys it lacks. This tool's *own* previous output deliberately does not count (`findSourceDossier`): a task that treated its own product as authored input would re-plan itself on every run. A second run is cheap for a different reason — `--resume` and `run.skipExistingOutputs` skip what is already on disk. The dry-run cost preview knows the difference: a task can declare `usesLlm: false` per-task, not just per-pipeline.
...
**`progress.log` — the one surface written to be read *while* the run is going** (`src/observability/ProgressLog.ts`, `logging.progressFile`). Everything else this tool records is for a machine to read back afterwards: `events.jsonl` is an audit trail, `state.json` a checkpoint, and both are complete and both are unreadable while you are waiting four hours. This is one plain line per finished task in the project root, appended, `tail -f`-able:
...
- **The model has to be joined to the task.** Only the gateway knows which target answered and only the orchestrator knows a task is finished; the key between them is the task id, which every completion request already carries as its `correlationId`. `AttemptRecord` now carries it, plus the wire `modelName`. The *last success* wins, so a task that fell back is credited to the model that actually did the work.
- **A line with `!` in it is an incident** - a retry, a fallback, or a target written off for the run - and it exists so the line under it is explicable: the file that says `or-luna` was meant for `local-small`, and the line above says why. `grep ' ! ' progress.log` is the incident report. Incidents name their task (`ProgressLog.taskStarted` remembers the label) because the file they are about does not exist yet and may never.
- **Which file a line is about** is not simply "the outputs": `extract` writes a dossier *and* an internal `.hints/` hand-off, and only the first is worth a line - but `portrait`'s whole product *is* a hint file. The rule is published outputs, else whatever was written, else the task's own label (a web search with no gaps, or a failure).
- **Writes are buffered and flushed at most once per `logging.progressIntervalMs`**, which is equally a floor and a ceiling: a line arriving just after a write is flushed when the interval elapses rather than waiting for the next task, or a slow document leaves the file looking stalled. `--dry-run` writes nothing - a log of what processed a corpus that was not processed would be fiction.

**`ObserverHub` dropped `onTargetDown` entirely**, and nothing noticed because `GatewayObserver`'s methods are optional: the hub is what the gateway actually talks to, so an event it does not forward reaches no listener. Every mechanism built on it - the metrics counter, the run summary's named call-out for a target that served zero requests, the `llm.target_down` journal record - was dead from the day it was written. This is the failure mode the section below describes, arriving one level lower than expected.
