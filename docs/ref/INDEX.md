...
| [adaptive-routing.md](adaptive-routing.md) | the `adaptive` strategy: its five inputs, its two subjective numbers, why a share target is not always reachable, and the local harness that replaced paid experiments — including the day the harness turned out to be measuring nothing |
